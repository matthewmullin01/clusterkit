...
RSpec.describe AnnEmbed::Silence do
  ...
  describe 'UMAP integration' do
    it 'respects verbose configuration' do
      ...
      AnnEmbed.configuration.verbose = false
      expect(AnnEmbed.configuration.verbose).to be false
      
      # When verbose is false, should use silence
      expect(AnnEmbed::Silence).to receive(:silence_output).and_call_original
